  ...
  mutations: {
    login: (state: AuthState, payload: UserLoggedResponse) => {
      ...
      if (payload.user.locale) {
        ...
        i18n.locale = payload.user.locale;
      }
      ...
      if (router.currentRoute.query.redirect) {
        router.push(router.currentRoute.query.redirect.toString());
      } else {
        if ((payload.user as UserDto).type === UserType.Admin && process.env.VUE_APP_SERVICE !== "sandbox") {
          router.push("/admin");
        } else {

  ...
  <div class="bg-white dark:bg-gray-900 pb-6">
    <div class="relative pt-6 z-40">
        ...
        >
              ...
              <div class="-mr-1 flex items-center md:hidden">
                <div class="flex">
                  <div class="inline-flex rounded-md space-x-2">
                    <LocaleSelector
                      class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-sm text-gray-900 dark:text-blueGray-300"
                    />
                    <router-link
          ...
          >
            <span class="inline-flex rounded-md shadow">
              <router-link
                :to="$store.state.auth.authenticated ? '/app' : '/login'"
                class="inline-flex items-center px-4 py-2 border border-transparent text-base font-medium rounded-sm text-theme-600 dark:text-white bg-white dark:bg-blueGray-800 hover:bg-gray-50"
              >
                <div v-if="!$store.state.auth.authenticated">{{ $t("account.shared.signIn") }}</div>
                <div v-else>{{ $t("shared.enter") }}</div>
              </router-link>
      ...
      >
        ...
        >
          <div
            class="rounded-lg bg-blueGray-50 dark:bg-blueGray-800 border border-gray-200 dark:border-gray-700 shadow-xl ring-1 ring-black ring-opacity-5 overflow-hidden"
          >
            ...
            <div role="menu" aria-orientation="vertical" aria-labelledby="main-menu">
              <div class="px-2 pt-2 pb-3" role="none">
                <router-link
                  ...
                  :key="idx"
                  :to="link.path"
                  class="block px-3 py-2 rounded-md text-base font-medium text-gray-900 dark:text-blueGray-300 hover:text-gray-900 hover:bg-gray-50 dark:hover:bg-blueGray-800"
                  role="menuitem"
                  :class="isCurrent(link.path) ? 'bg-blueGray-100 dark:bg-coolGray-900' : ''"
                >{{ link.title }}</router-link>
              </div>
              <div role="none" class="flex space-x-2 items-center">
                <router-link
                  to="/register"
                  class="block w-full px-5 py-3 text-center font-medium text-gray-900 dark:text-blueGray-300 bg-blueGray-50 dark:bg-blueGray-800"
                  role="menuitem"
                >

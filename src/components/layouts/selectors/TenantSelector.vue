  ...
  <div class="relative shadow-2xl" v-click-outside="closeDropdownTenant">
    <span class="inline-block w-full rounded-sm shadow-sm">
      <button
        ...
        class="bg-blueGray-800 hover:bg-theme-700 hover:text-theme-50 truncate text-blueGray-300 cursor-pointer w-full pl-3 py-2 text-left focus:outline-none transition ease-in-out duration-150 sm:leading-5 rounded-sm shadow-sm pr-7"
      >
        <span class="font-bold">{{ currentTenant.name }}</span>
        <span class="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
          <svg
            class="h-5 w-5 text-blueGray-200"
            viewBox="0 0 20 20"
            fill="none"
            stroke="currentColor"
          >
      ...
      >
        ...
        >
          ...
          <button
            @click="addTenant"
            role="option"
            class="w-full text-left font-bold text-blueGray-800 cursor-pointer select-none relative py-2 pl-3 pr-9 hover:bg-gray-100 focus:outline-none"
...
export default class TenantSelector extends Vue {
  ...
  getPlanFromTenant(tenant: TenantDto): string {
    if (tenant.products && tenant.products.length > 0 && tenant.products[0].subscriptionProduct) {
      return this.$t(tenant.products[0].subscriptionProduct.title).toString();
    } else {
      if (tenant.subdomain === "admin") {
        return "Admin";
      } else {
        return this.$t("settings.subscription.notSubscribed").toString();
      }
    }
  }

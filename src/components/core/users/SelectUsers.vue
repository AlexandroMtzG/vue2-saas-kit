              ...
              <div class="max-w-lg mx-auto">
                <div>
                  <div class="text-center">
                    ...
                    >{{ $t("app.workspaces.actions.selectWorkspaceUsers") }}</h2>
                    <p
                      class="mt-1 text-sm text-gray-500"
                    >{{ $t("app.workspaces.actions.selectUsers") }}</p>
                  </div>
                  <form v-if="allowSearch" action="#" class="mt-6 flex">
...
export default class SelectUsers extends Vue {
  ...
  reload() {
    ...
    services.tenantUsers
      .getAll()
      .then((response) => {
        this.items = response.map((f) => f.user);
      })
      .catch((error) => {
  ...
  get filteredItems(): UserDto[] {
    ...
    const items = this.items.filter((f) => f.id?.toUpperCase().includes(this.searchInput.toUpperCase()) || f.email?.toString().toUpperCase().includes(this.searchInput.toUpperCase()) || f.firstName?.toString().toUpperCase().includes(this.searchInput.toUpperCase()) || f.lastName?.toString().toUpperCase().includes(this.searchInput.toUpperCase()));
